...
export default function LandingPage({ onEnter, onAccessPortal }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position:'relative', zIndex:2, width:'100%', maxWidth:820, padding: isMobile ? '0 16px' : '0 24px' }}>
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.1}}
          style={{display:'flex',alignItems:'center',gap:12,marginBottom:isMobile ? 12 : 24}}>
          <div style={{width:isMobile ? 24 : 36,height:isMobile ? 24 : 36,border:'2px solid var(--red)',display:'flex',alignItems:'center',justifyContent:'center',transform:'rotate(45deg)',flexShrink:0}}>
            <span style={{transform:'rotate(-45deg)',color:'var(--red)',fontSize:isMobile ? 12 : 16,lineHeight:1}}>⬡</span>
          </div>
          <div>
            <div style={{fontFamily:'var(--orb)',fontSize:isMobile ? 8 : 10,letterSpacing:'.35em',color:'var(--cyan)',textTransform:'uppercase'}}>HAWKINS NATIONAL LABORATORY</div>
            <div style={{fontFamily:'var(--share)',fontSize:isMobile ? 7 : 9,letterSpacing:'.2em',color:'var(--phosphor-d)',textTransform:'uppercase',marginTop:2}}>CLASSIFIED NETWORK — AUTHORIZED PERSONNEL ONLY</div>
          </div>
        </motion.div>

        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.8,delay:.3}}
          style={{marginBottom:isMobile ? 20 : 32,position:'relative'}}>
          <AnimatePresence>
            {glitch && (
              <motion.div key="glitch-wrap">
                <motion.h1 key="g1" initial={{opacity:0}} animate={{opacity:.45}} exit={{opacity:0}}
                  style={{position:'absolute',top:2,left:3,fontFamily:'var(--orb)',fontWeight:900,fontSize: isMobile ? '2.5rem' : 'clamp(2.5rem,7vw,5.5rem)',textTransform:'uppercase',letterSpacing:'.02em',color:'var(--cyan)',pointerEvents:'none',lineHeight:1}}>
                ...
                <motion.h1 key="g2" initial={{opacity:0}} animate={{opacity:.35}} exit={{opacity:0}}
                  style={{position:'absolute',top:-2,left:-3,fontFamily:'var(--orb)',fontWeight:900,fontSize: isMobile ? '2.5rem' : 'clamp(2.5rem,7vw,5.5rem)',textTransform:'uppercase',letterSpacing:'.02em',color:'var(--purple)',pointerEvents:'none',lineHeight:1}}>
                  HELLFIRE<br/><span style={{fontSize:'60%',letterSpacing:'.15em',fontWeight:400}}>LEDGER RESOLVER</span>
                </motion.h1>
              </motion.div>
            )}
          </AnimatePresence>
          <h1 className="glow-red" style={{fontFamily:'var(--orb)',fontWeight:900,fontSize: isMobile ? '2.5rem' : 'clamp(2.5rem,7vw,5.5rem)',textTransform:'uppercase',letterSpacing:'.02em',lineHeight:1}}>
            HELLFIRE<br/>
        ...
        <div className="ticker-strip" style={{borderTop:'1px solid var(--b-phosphor)',borderBottom:'1px solid var(--b-phosphor)',padding:'6px 0',marginBottom:isMobile ? 12 : 24,background:'rgba(179,255,179,.025)'}}>
        ...
        <div className="panel panel-screws" style={{marginBottom:isMobile ? 24 : 32,maxHeight:isMobile ? 200 : 280,overflow:'hidden'}}>
          <div className="panel-label panel-label--cyan">
            <span className="status-dot status-dot--cyan"/>
            {isMobile ? 'BOOT SEQ' : 'SYSTEM BOOT SEQUENCE — TERMINAL 0x04'}
          </div>
          <div ref={termRef} style={{padding:'12px 16px',maxHeight:isMobile ? 160 : 232,overflowY:'auto',background:'var(--bg-term)'}}>
            {lines.map((ln, i) => (
              <motion.div key={i} initial={{opacity:0,x:-6}} animate={{opacity:1,x:0}} transition={{duration:.2}}
                style={{display:'flex',alignItems:'center',gap:8,marginBottom:2}}>
                <span className={colClass[ln.col] || 'term-line'} style={{fontSize:isMobile ? 10 : 12}}>{ln.txt}</span>
                {ln.ok && <span style={{fontFamily:'var(--vt)',fontSize:isMobile ? 14 : 18,color:'var(--green)',marginLeft:'auto',whiteSpace:'nowrap'}}>[OK]</span>}
              </motion.div>
            ))}
            {showCTA && (
              <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{display:'flex',alignItems:'center',gap:4,marginTop:6}}>
                <span className="term-line term-line--red" style={{fontSize:isMobile ? 10 : 12}}>{'>'}</span>
                <span className="term-line" style={{fontSize:isMobile ? 10 : 12}}>AWAITING CLEARANCE</span>
                <span className="blink" style={{fontFamily:'var(--vt)',fontSize:isMobile ? 18 : 22,color:'var(--phosphor)'}}>█</span>
              </motion.div>
            )}
          </div>
        </div>
        ...
        <AnimatePresence>
          {showCTA && (
            <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:.6}}
              style={{display:'flex',flexDirection: isMobile ? 'column' : 'row', gap:16, alignItems:'center'}}>
              <motion.button
                ...
                data-hover="true"
                style={{
                  fontFamily:'var(--orb)',fontWeight:700,fontSize:isMobile ? 10 : 12,
                  padding:isMobile ? '12px 24px' : '14px 36px',
                  letterSpacing:'.15em', width: isMobile ? '100%' : 'auto'
                }}>
                ◈ ENTER CONTROL PANEL
              </motion.button>
              <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.3}}
                style={{fontFamily:'var(--share)',fontSize:isMobile ? 8 : 9,letterSpacing:'.2em',color:'var(--phosphor-d)',textTransform:'uppercase'}}>
      ...
      <div style={{
        ...
        <div style={{display:'flex',gap:16,alignItems:'center'}}>
          <span style={{display:'flex',gap:6,alignItems:'center',fontFamily:'var(--share)',fontSize:isMobile ? 7 : 9,letterSpacing:'.2em',color:'var(--red)',textTransform:'uppercase'}}>
            <span className="status-dot status-dot--red"/>
            {isMobile ? 'KLINE: ACT' : 'KLINE EVASION: ACTIVE'}
          </span>
        </div>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <span className="status-dot status-dot--green"/>
          <span style={{fontFamily:'var(--share)',fontSize:isMobile ? 7 : 9,letterSpacing:'.15em',color:'var(--phosphor-d)',textTransform:'uppercase'}}>
            {isMobile ? 'NOMINAL' : 'UPLINK: NOMINAL'}
          </span>
        </div>
      </div>
    </div>
