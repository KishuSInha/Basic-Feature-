...
import { BadgeCheck, Send, Zap, ChevronDown, User, Filter, AlertCircle } from 'lucide-react';
...
const SettlementList = ({ settlements = [], stats = null }) => {
  ...
  const filterLabels = {
    all: 'Show All Settlements',
    mine: 'Show My Settlements',
    settled: 'Settled Data Only',
    remaining: 'Remaining Data'
  };

  return (
    <div className="flex flex-col gap-4 h-full overflow-hidden pr-3 pb-8">
      ...
      <div className="flex gap-2 z-30">
        <div className="relative flex-1 group">
           ...
           <input 
              type="text"
              placeholder="WHO ARE YOU? (NAME SEARCH)"
              value={userName}
        ...
        <div className="relative">
           <button 
              ...
              className="px-4 py-2 bg-black/40 border border-phosphor/20 text-phosphor font-share-tech text-[10px] tracking-widest flex items-center gap-2 hover:border-phosphor transition-all min-w-[160px] justify-between"
           >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 flex flex-col gap-4">
        {stats && filter === 'all' && (
          <div className="mb-2 p-4 border border-phosphor/20 bg-phosphor/5 shrink-0">
             <div className="flex justify-between items-center text-[10px] font-share-tech">
                <div>
                   <p className="text-phosphor/60">SYSTEM STATUS: <span className="text-neon-cyan brightness-150">OPTIMIZED</span></p>
                   <p className="text-phosphor/40 mt-1">SAVED: <span className="text-white">{stats.savings_pct}</span></p>
                </div>
                <div className="text-right">
                   <p className="text-white uppercase tracking-tighter">{stats.raw_transactions} RAW → {stats.optimized_settlements} SETTLEMENTS</p>
                   <p className="text-phosphor/30 text-[8px] mt-1">HNL PROTOCOL LAYER 4</p>
                </div>
             </div>
          </div>
        )}
        ...
        <AnimatePresence mode="popLayout">
          {filteredSettlements.map((item, localIdx) => {
            // We need a stable global ID based on the original index in the settlements array
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-orbitron font-bold tracking-[0.2em] text-phosphor/80 uppercase">UPLINK DIRECTIVE #{globalIdx+1}</span>
                  </div>
                ...
                <div className="flex items-center justify-between p-3 bg-bg-dark/50 border-y border-phosphor-dim/10 mb-4 group-hover:bg-bg-dark/80 transition-colors">
                  <div className="text-center w-1/3">
                    ...
                    <p className="text-[10px] font-orbitron text-white truncate px-1 uppercase">{item.from}</p>
                  </div>
                  ...
                  <div className="text-center w-1/3">
                    ...
                    <p className="text-[10px] font-orbitron text-white truncate px-1 uppercase">{item.to}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2 relative z-10">
                  {currentStatus === 'PENDING' ? (
                    <button 
                      ...
                      className="flex-1 py-2 bg-bg-dark border border-phosphor text-phosphor font-orbitron text-[10px] tracking-widest hover:bg-phosphor hover:text-bg-dark transition-all flex items-center justify-center gap-2 group/btn uppercase"
                    >
                      <Send className="w-2.5 h-2.5 group-hover/btn:translate-x-1 transition-transform" />
                      EXECUTE SETTLEMENT
                    </button>
                  ) : currentStatus === 'CONFIRMED' ? (
                    <div className="flex-1 border border-phosphor/30 bg-phosphor/5 py-2 text-[10px] text-phosphor font-orbitron flex items-center justify-center gap-2 uppercase tracking-widest">
                      <BadgeCheck className="w-3.5 h-3.5" />
                      SETTLED IN BLOCKCHAIN
                    </div>
                  ) : (
                    <div className="flex-1 border border-neon-cyan py-2 text-[10px] text-neon-cyan font-orbitron flex items-center justify-center gap-3 uppercase tracking-widest">
